...
At the end of this work, we were allowed to use the computing grid of the Algorithms and Data Structures Group of the Vienna University of Technology to test our program. All test instances are taken from \cite{Noronha2006}. Each instance of the program was started with a time limit of 600s and a limit of 5000 unsusccessfull shaking runs. The results of the tests can be found in table~\ref{tab:result}.
...
\begin{description}
    \item[Instanz] Name of the computed instance.
    ...
    \item[$N$] Used Neighborhoods in order of iteration:
        ...
        \end{description}
    \item[$S_{avg}$] The average number of colors used by our program. Every test instance was run 40 times to negate statistical anomalies.
    \item[$S_{\sigma}$] The mean deviation of our results. Due to randomisation happening in each run of the 40 runs, anomlies may occur.
...
It can be seen that in order to get the best results, multiple orders of Neighborhoods in the VND are possible. An overall trend is the average improvement on huge instances around 10\% to 15\%. All these improvements are measured relative to the construction heuristic \emph{onestepCD}.
...
In all three videos the instance \texttt{n20p5t2s5.pcp} is shown. The reason for that is, that bigger instances with more vertices tend to get confusing very quickly.
