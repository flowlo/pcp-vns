...
\subsection{Problem}
In technical publication the problem is known as Partition Graph Coloring Problem (PCP). PCP is an optimization problem in the realm of network design or rather in the realm of planning the communication in an all-fiber-optic network. In most fiber-optic networks Wavelength Division Multiplexing (WDM) is used to transmit two or more communication channels over the same optic fiber. This is possible by using different wavelengths for each channel, as long as these wavelengths are sufficiently destinguishable. To minimize the complexity of a single switch within the network a single communication channel uses only one wavelength on its way through the network.
...
This optimization problem can be seen as a generalization of the well-known Graph Coloring Problem (GC). Instead of coloring a single vertex, which is required by the GC, a whole cluster of vertices must be colored: for each communication that is known in advance, a selection of possible routes through the network is computed (for example based on the route length or bandwidth). Each communication represents a cluster of different vertices and each vertex of which represents a different route through the network. In the resulting graph -- also referred to as conflict graph -- every vertex has an edge to another vertex when the routes they represent share the same optical fiber somewhere in the network. These edges indicate that these two vertices must not share the same color, as this would result in an communication error (see figure \ref{fig:translation}). To solve the problem, one representative vertex has to be selected for each cluster, this way selecting a route through the network. Additionally all selected vertices must be colored in such a way that no two vertices connected by the same edge use the same color (the color represents the wavelength used for a communciation). Simultaneously trying to minimize the use of different colors, which leads to fewer used wavelenghts, is the optimization problem known as Partition Graph Coloring Problem. It is known to be NP-hard.
...
This very simple Neighborhood was inspired by the Tabu Search from \citet*{Noronha2006}. The Neighborhood tries to recolor all vertices with the heighest color.
...
To reach its goal, the Neighborhood first selects a vertex colored with the heighest color. This vertex is then recolored with the lowest color, even if this leads to conflicts in the solution. The algorithm then proceeds to find new suitable colors for all adjacent vertices of the selected vertex. 
