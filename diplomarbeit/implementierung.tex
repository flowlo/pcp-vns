...
die Übergabe der vollständigen, unveränderten Probleminstanz \texttt{orig}. Auch von essentieller Bedeutung ist der String \texttt{units}, welcher eine Zeichenkette enthält, der die Reihenfolge der verwendeten
...
Die letzten beiden Parameter dienen der Einstellung ob Zwischenlösungen (\texttt{checkIntermediate}) oder die endgültige Lösung (\texttt{checkFinal}) durch die Plausibilitätsprüfung überprüft werden sollen.
...
Bei dieser Nachbarschaft wurde versucht den in Abschnitt \ref{sec:ChangeColor} vorgeschlagene Ansatz in eine konkrete Nachbarschaft zu übersetzen. Da sich bei Tests herrausstellte, dass der beschriebene Ansatz
...
Verbesserung bringt, wird die Nachbarschaft abgebrochen. Konnten für alle Knoten, welche ursprünglich die höchste Farbe besaßen, eine Ersatzfarbe gefunden werden, wird das neue Solutionobjekt zurückgegeben, 
...
Die Nachbarschaft ist unter Algorithmus~\ref{psy:ChangeNode} zu finden. Ähnlich wie bei ChangeColor wird versucht, alle Knoten mit der maximalen Farbe umzufärben. Bei ChangeNode wird dies versucht, in dem
...
neu einzufärben. Sollte die Neueinfärbung mit einer Farbe niedriger als der maximalen Farbe erfolgen, wird nach weiteren Knoten maximaler Farbe gesucht. Sollte der Algorithmus nicht in der Lage sein, ein 
...
\State Färbe Knoten $a$ mit minimaler Farbe ein
\If{$a.farbe < c$}
\State Beende innere Schleife  
\EndIf
\EndFor
...
zu können. In Test bei großen und kleinen Probleminstanzen zeigte sich, das ChangeAll zu Verbesserungen führt, welche weder von ChangeNode noch von ChangeColor vorgenommen wurden. 
