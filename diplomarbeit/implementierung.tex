...
Außerdem besitzt Solution mehrere Methoden, einen Konfliktgraphen aus einer Datei auszulesen, und diesen wiederum in einem Solution-Objekt zu speichern. Diese Methoden zum Einlesen von Dateien werden genauer 
...
Für die Speicherung von sowohl Lösungs- als auch Konfliktgraphen dient die \texttt{adjacency\_list}-Datenstruktur, welche von Boost bereitgestellt wird. Der Einsatz von Boost wird in Abschnitt~\ref{sec:boost} 
...
der innerhalb einer Solution gespeicherten Eigenschaften, und an Hand des ursprünglichen Konfliktgraphen fest, ob es sich um eine valide Lösung handelt oder nicht. Diese Überprüfung erfolgt standardmäßig
...
Testungen auch ein zweites Format zum einlesen eines Konfliktgraphen implementiert. 
...
die Anzahl der Kanten, und die Anzahl der Partition im Konfliktgraph, genau in jener Reihenfolge. Nun folgen \textit{n} Zeilen an Zahlen, mit 
...
Datei von einem Eingabestream ein, und wandelt sie in ein Solutionobjekt mit entsprechendem Konfliktgraphen um. Dieses Solutiobjekt wird während
...
erzeugt ein neues Solutionobjekt, als einziger Übergabeparameter ist das aus der Datei eingelesen Solutionobjekt, welches ja den vollständigen Konfliktgraphen
